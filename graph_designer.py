...
def init_graph_html():
    ...
    txt = ""
    for node in v:
        txt += f"<div class='node' id='{node}' style='top:{nodes_positions[node]['y']+40}%;left:{nodes_positions[node]['x']+20}%;' ondblclick='javascript:change(this.id);'>{node}</div>\n"
    txt += "<script>\n"
    txt += "  $('.node').draggable();\n"      
